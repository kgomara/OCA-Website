...
The site was migrated from older Bootstrap 2/3 assets to Bootstrap 5.3.8. Bootstrap and jQuery are loaded from public CDNs; project-specific styling and behavior live in local files under `WebContent`.
...
Because the site is static, it can be previewed directly from `WebContent/index.html`. A local web server is useful for testing browser behavior consistently:
...
- Open the repository root, not `WebContent`, in VS Code.
- The repository root is the folder containing this README and the top-level `.git` directory.
- `WebContent` is a normal tracked folder, not a separate Git repository.
